...
export async function processCSVWithDuckDB(
  ...
  try {
    ...
    try {
      ...
      // For very large files, we need special handling
      // JavaScript's arrayBuffer() has limitations - browsers typically can't handle >2-4GB
      // DuckDB WASM requires files to be in memory as Uint8Array for registration
      // 
      // CRITICAL: Browsers fundamentally cannot load 10GB+ files into memory
      // Even with chunked reading, we need the complete file in memory for DuckDB registration
      // This is a browser limitation, not a DuckDB limitation
      const MAX_MEMORY_FILE_SIZE_GB = 2.0; // Reduced from 3.5GB - be more conservative
      let uint8Array: Uint8Array;
      
      if (fileSizeGB > MAX_MEMORY_FILE_SIZE_GB) {
        // Files > 2GB cannot be loaded into memory reliably in browsers
        // This is a fundamental browser limitation - we cannot process such large files client-side
        console.error(`❌ File too large (${fileSizeGB.toFixed(2)}GB) for in-browser processing`);
        ...
        throw new Error(
          ...
          `Your file is ${fileSizeGB.toFixed(2)}GB, but browsers can only handle files up to ~${MAX_MEMORY_FILE_SIZE_GB}GB due to memory limitations.\n\n` +
          `💡 Solutions:\n` +
          `1. Split the file into smaller chunks (<${MAX_MEMORY_FILE_SIZE_GB}GB each)\n` +
          `2. Use a random sample of rows instead of the full dataset\n` +
          `3. Filter/process the file externally first\n` +
          `4. Use server-side processing (e.g., DuckDB CLI, Python)\n\n` +
          `Why? DuckDB WASM requires loading the entire file into browser memory, which is impossible for files this large.`
        );
      }
      ...
      try {
        ...
        onProgress?.({ file: fileName, percent: 30, message: 'Importing CSV into DuckDB...' });
        
        // Use insertCSVFromPath which is optimized for large files
        await conn.insertCSVFromPath(safeFileName, {
          schema: 'main',
          name: tableName,
          detect: true,
          header: true
        ...
        DEBUG && console.log(`✅ CSV imported into table`);
